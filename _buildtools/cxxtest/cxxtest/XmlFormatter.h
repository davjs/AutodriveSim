...
namespace CxxTest
{
    ...
    class XmlFormatter : public TestListener
    {
        ...
        {
            (*_o) << "<world>" << endl;
			getTimeStamp(buffer, 80);
            (*_o) << "    <started timestamp=\"" << buffer << "\"/>" << endl;
            _o->flush();
        }
        ...
        {
	    		getTimeStamp(buffer, 80);
                (*_o) << "        <finished timestamp=\"" << buffer << "\"/>" << endl;
                (*_o) << "    </testsuite>" << endl;
                _o->flush();
        }
        ...
        void leaveWorld( const WorldDescription &desc )
        {
                ...
                char tmpStr[WorldDescription::MAX_STRLEN_TOTAL_TESTS];
                const WorldDescription &wd = tracker().world();
                (*_o) << "    <summary>" << endl;
                (*_o) << "    	<totaltests>" << wd.strTotalTests( tmpStr ) << "</totaltests>" << endl;
                (*_o) << "    	<failedtests>" << tracker().failedTests() << "</failedtests>" << endl;
                unsigned numPassed = desc.numTotalTests() - tracker().failedTests();
                (*_o) << "    	<successrate>" << (numPassed * 100 / desc.numTotalTests()) << "</successrate>" << endl;
                (*_o) << "    </summary>" << endl;
                (*_o) << "</world>" << endl;
                _o->flush();
        }
    ...
    private:
        ...
        void getTimeStamp(char *_buffer, unsigned int max)
        {
        	time_t myTime;
			time(&myTime);
			...
			strftime(_buffer, max, "%Y-%m-%d %H:%M:%S", myTm);
        }
        ...
        void testFailure( const char *file, unsigned line, const char *failureType, const char *message = NULL )
        {
            ...
            lineStream << line;
            output << attributeText( "line", lineStream.str().c_str() );
        ...
        void dump( const void *_buffer, unsigned size )
        {
            unsigned dumpSize = size;
            if ( maxDumpSize() && dumpSize > maxDumpSize() )
                dumpSize = maxDumpSize();

            const unsigned char *p = (const unsigned char *)_buffer;
