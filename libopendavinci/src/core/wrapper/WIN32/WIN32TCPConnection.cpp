        ...
        namespace WIN32Impl {
            ...
            WIN32TCPConnection::WIN32TCPConnection(const std::string& ip, const uint32_t &port) :
                ...
                if ( ::getaddrinfo(ip.c_str(), sstr.c_str(), &hints, &res) == -1 ) {
					...
					stringstream s;
					const int retcode = WSAGetLastError();
					s << "[WIN32TCPConnection] Error while getting info at " << __FILE__ << ": " << __LINE__ << ": " << retcode;
                ...
                if (m_fileDescriptor < 0) {
					...
					stringstream s;
					const int retcode = WSAGetLastError();
					s << "[WIN32TCPConnection] Error creating socket at " << __FILE__ << ": " << __LINE__ << ": " << retcode;
                ...
                if ( ::connect(m_fileDescriptor, res->ai_addr, res->ai_addrlen) < 0 ) {
					...
					stringstream s;
					const int retcode = WSAGetLastError();
					s << "[WIN32TCPConnection] Error connecting to socket at " << __FILE__ << ": " << __LINE__ << ": " << retcode;
            ...
            void WIN32TCPConnection::initialize() {
                ...
                if (m_thread.get() == NULL) {
                    throw std::string("[WIN32TCPConnection] Error creating thread");
                }

                m_socketMutex = auto_ptr<Mutex>(MutexFactory::createMutex());
                if (m_socketMutex.get() == NULL) {
                    throw std::string("[WIN32TCPConnection] Error creating mutex for socket");
                }
            }
        }
    }
