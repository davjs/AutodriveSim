...
namespace context {
    namespace base {
        ...
        RuntimeEnvironment::RuntimeEnvironment() :
            m_listsMutex(),
            m_listOfConferenceClientModules(),
        ...
        RuntimeEnvironment::~RuntimeEnvironment() {
            Lock l(m_listsMutex);
            m_listOfConferenceClientModules.clear();
            m_listOfSystemFeedbackComponents.clear();
            m_listOfSystemReportingComponents.clear();
        }

        void RuntimeEnvironment::add(ConferenceClientModule &ccm) {
            if (!isExecuting()) {
                Lock l1(m_executingMutex);
                {
                    Lock l2(m_listsMutex);
                    m_listOfConferenceClientModules.push_back(&ccm);
                }
            }
        }
        ...
        bool RuntimeEnvironment::isValid() const {
            bool retVal = false;
            {
                ...
                retVal = ( (m_listOfConferenceClientModules.size() > 0) && (m_listOfSystemFeedbackComponents.size() > 0) );
        ...
        vector<core::base::ConferenceClientModule*> RuntimeEnvironment::getListOfConferenceClientModules() {
            Lock l(m_listsMutex);
            return m_listOfConferenceClientModules;
        }
        ...
        uint32_t RuntimeEnvironment::getGreatestTimeStep() const {
            ...
            vector<ConferenceClientModule*>::const_iterator it = m_listOfConferenceClientModules.begin();
            while (it != m_listOfConferenceClientModules.end()) {
