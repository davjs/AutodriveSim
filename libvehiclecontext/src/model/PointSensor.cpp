...
namespace vehiclecontext {
    namespace model {
        ...
        PointSensor::PointSensor(const uint16_t &id, const string &name, const core::data::environment::Point3 &translation, const double &rotZ, const double &angleFOV, const double &distanceFOV, const double &clampDistance, const bool &showFOV, const double &faultModelSkip, const double &faultModelNoise) :
            m_id(id),
            m_name(name),
...
//           cerr << m_name << " angleDelta: " << angleDelta << ", totalRotation: " << m_totalRotation << ", left: " << (m_totalRotation + (m_angleFOV/2.0)*Constants::DEG2RAD) << ", right: " << (m_totalRotation - (m_angleFOV/2.0)*Constants::DEG2RAD) << ", " << (m_totalRotation + angleDelta) << endl;

            // Check if the point is within the FOV.
            if ( ( ((m_totalRotation + angleDelta) > (m_totalRotation - (m_angleFOV/2.0)*Constants::DEG2RAD)) &&
                   ((m_totalRotation + angleDelta) < (m_totalRotation + (m_angleFOV/2.0)*Constants::DEG2RAD)) ) || 
                 ( (fabs((m_totalRotation - (m_angleFOV/2.0)*Constants::DEG2RAD) - (m_totalRotation + angleDelta)) < 1e-4) ||
                   (fabs((m_totalRotation + (m_angleFOV/2.0)*Constants::DEG2RAD) - (m_totalRotation + angleDelta)) < 1e-4) )
               ) {
                // Distance found.
