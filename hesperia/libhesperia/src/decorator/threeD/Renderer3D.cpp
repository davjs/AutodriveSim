    ...
    namespace decorator {
        namespace threeD {
            ...
            void Renderer3D::drawTriangleSet(const TriangleSet &ts, const Point3 &position, const Point3 &rotation) {
                ...
                glRotatef(static_cast<float>(rotation.getX()*180.0 / Constants::PI), 1, 0, 0);
                glRotatef(static_cast<float>(rotation.getY()*180.0 / Constants::PI), 0, 1, 0);
                glRotatef(static_cast<float>(rotation.getZ()*180.0 / Constants::PI), 0, 0, 1);
            ...
            void Renderer3D::drawListOfTriangleSets(const vector<hesperia::decorator::models::TriangleSet> &listOfTriangleSets, const core::data::environment::Point3 &position, const core::data::environment::Point3 &rotation) {
                glTranslatef(static_cast<float>(position.getX()), static_cast<float>(position.getY()), static_cast<float>(position.getZ()));
                ...
                glPushMatrix();
                {
                    ...
                    Point3 rotateAroundXAxis(core::data::Constants::PI/2.0, 0, 0);

                    glRotatef(static_cast<float>(rotateAroundXAxis.getX()*180.0 / Constants::PI), 1, 0, 0);
                    glRotatef(static_cast<float>(rotateAroundXAxis.getY()*180.0 / Constants::PI), 0, 1, 0);
                    glRotatef(static_cast<float>(rotateAroundXAxis.getZ()*180.0 / Constants::PI), 0, 0, 1);
            ...
            void Renderer3D::drawImage(const core::wrapper::Image *image, const Point3 &originPixelXY, const Point3 &scalingPixelXY, const float &rotationZ) {
                if (image != NULL) {
                    ...
                    Point3 flipAroundXAxis(Constants::PI, 0, rotationZ);
                    ...
                    glPushMatrix();
                    {
                        ...
                        glPushMatrix();
                        {
                            ...
                            {
                                // Rotate the model using DEG (rotation is in RAD!).
                                glRotatef(static_cast<float>(flipAroundXAxis.getX()*180.0 / Constants::PI), 1, 0, 0);
                                glRotatef(static_cast<float>(flipAroundXAxis.getY()*180.0 / Constants::PI), 0, 1, 0);
                                glRotatef(static_cast<float>(flipAroundXAxis.getZ()*180.0 / Constants::PI), 0, 0, 1);
