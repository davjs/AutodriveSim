...
namespace hesperia {
    namespace data {
        namespace scenario {
            ...
            const vector<core::data::environment::Point3> Polygon::getListOfVerticesAsPoint3() const {
                vector<core::data::environment::Point3> result;

                vector<Vertex3>::const_iterator it = m_listOfVertices.begin();
                while (it != m_listOfVertices.end()) {
                    core::data::environment::Point3 p = (*it++);
                    result.push_back(p);
                }
